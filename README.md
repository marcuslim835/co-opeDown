...
## 1 Quick Start Guide
For Windows Users: 
1) Download the game by [clicking here](https://github.com/marcuslim835/co-opeDown/releases/download/v1.0/Windows.zip) <br>
...
For Mac Users: 
1) Download the game by [clicking here](https://github.com/marcuslim835/co-opeDown/releases/download/v1.0/MacOS.zip) <br>
2) After download, extract the files from the zip folder and run co-opeDown. <br>
...
Feature | Description
------------ | -------------
Level Based Design | Provide a solid framework for the development of tasks that are increasing in difficulty. When contrasted to the difficulty levels, the step up in difficulty would be smaller. For example, higher levels of the same difficulty level would involve bigger numbers, but with the same nature of operations (eg. addition questions). Refer to the developer guide section 1.4 for a detailed explanation or the diagram below for a brief explanation.
Co-op | For two different people to work together to solve questions.
Infinite Replayability | Randomly generated questions to ensure every experience is an unique experience.
One-handed Controls | Set to four keys per controlled character to allow two players to use a single keyboard to play the game (and even for one person to use both his hands to control both characters!).
Save/Load System | Enables players to pick up and put down the game whenever they want to without losing progress.
Different Game Difficulties | Differing sets of problems for different age groups. For example, easier game difficulty levels can be math problems dealing with addition. A higher game difficulty level would include multiplication and even polynomials.
2 Game Modes | Classic mode for an all-round experience, Endless mode for those dedicated to only solving math problems.
Tutorial Mode | Tutorial mode for new players to be familiarised with the controls to start the game.
...
| 23  | Work from Home: Code for Different Difficulties for all existing stages    | 7/7/2021 - 10/7/2021  | 8               | 8               | 1) Added different question generation mechanics for the respective stages in classic mode and endless mode |
...
| 25  | Work from Home: Final Touch-up, Update User and Developer Guides           | 17/7/2021 - 26/7/2021 | 12              | 12              | 1) Update user and developer guide <br> 2) Migrate data over from google drive to github |
| 26  | User Testing: Design and Conduct User Testing 1                            | 17/7/2021 - 20/7/2021 | 7               | 7               | 1) Design questions and tasks to test games and verify problems <br> 2) Gather feedback from 5 pairs of users |
| 27  | Work from Home: Optimization using User Feedback and Bug Fixing            | 20/7/2021 - 21/7/2021 | 6               | 6               | 1) Analyzed user feedback <br> 2) Added tutorial to the game <br> 3) Modified instructions for Stage X-2 for clarity |
| 28  | User Testing: Design and Conduct User Testing 2                            | 21/7/2021 - 22/7/2021 | 4               | 4               | 1) Gather feedback from another 5 pairs of users |
| 29  | Work from Home: Optimization using User Feedback and Bug Fixing            | 22/7/2021 - 23/7/2021 | 4               | 4               | 1) Analyzed user feedback <br>    |
| 30  | Work from Home: Project Video, Poster, Log, and Readme                     | 24/7/2021 - 26/7/2021 | 10              | 10              | 1) Tidy up project log <br> 2) Update readme with testing information <br> 3) Update project video with gameplay |
...
| Suggestion | Response |
| ---------- | -------- |
| Point 1 (Level based design) and point 3 (Differing difficult levels) seem to overlap, please clarify in your readme. | We have differentiated the 2 whereby across the level, questions and tasks deal with bigger numbers. As for the different difficulty, there will be a distinct increase in cognitive engagement, requiring speedier/deeper thinking. We have attacked a diagram illustrating this in [Section 5.2](https://github.com/marcuslim835/co-opeDown#52-features) |
| Since this is a family game as well, the game can be made such that more than 2 players can play it together at the same time as well, hence solving the problem that you mentioned. | As we want to keep it easy to play so that all ages will not face any substantial learning 'bumps', we find the typical 2 players keyboard sharing experience to be the easiest to implement. Therefore, we made it to be a co-op 2 player game. To allow more players to play this game, we implemented highscore functionalities so that more people can still compete with each other, by means of beating highscores. At the same time, since we want this game to be played with a single keyboard, it is not very feasible for us to add controls for a third player in, as 3 people using 1 keyboard to play is rather tight on space. |
| More details on the tech stack can be shown to let use know what is going on behind the scenes. The flowchart does not really show how the different level of difficulty is going to be implemented and the infinite replayability feature. More details on these would be great! Poster can be better done with the features shown being more prominent. | We have done up a developer guide to introduce what we used to build our game while elaborating on our decision making process, our poster has also been overhauled to include more information since Milestone 2. |
| The poster could have included the features more clearly with details. As of now, I am unable to obtain much information from the poster without reading the README. | We share your concerns about the lack of information. Our updated poster has been reformatted such that there are more details and information on what this game is about. |
| I feel that these features right now are pretty easy to implement (aside from local multiplayer if you do something like LAN) so I do believe if you are aiming for Apollo 11 you can look into including a storyline and maybe add a social aspect to the game to compare scores with others from an online leaderboard! | Since Milestone 1, we have further included different types of mini games as the game progresses. There are interactive quizzes added so as to touch on the social aspect in a co-op game. |
| However, one thing I can add is to playtest alot and early into the beginning phase. For any game, you need to have constant feedback and playtesting would accomplish just that. | We have done user testing and qualitative testing to make our game flow better and how our game can improve from a consumer point of view. |
| Infinite replayability with randomly generated questions: If these questions are randomly generated, how many questions do the group intend to make? The amount of work needed seems to be never-ending and countless. Perhaps refining this would be a bit more plausible. | Our questions have a large enough pool to generate from and questions can also be randomly generated and produced through wishful thinking. Many questions are math based, which means that infinite replayability using question generation via multiple Random.Int functions is possible. For more information on how we have done this, you can refer to our developer guide. |
| Bugs for current build: Sometimes just being in the vicinity of the number platforms without standing on them will already trigger the game to end. Can be solved by moving the Box Collider 2D components so no problem. You may want to take a look at the OnCollisionStay2D function vs OnCollisionEnter2D function if you are using Unity version 2020. | We have re-edited the platforms to be more detailed and responsive to any movements of the players. |
| Since Unity does not have a global variable system (scenes are discrete) here's a script that allows you to transfer objects that you want to be constantly running between scenes. I used it for a continuous music player in my game. >Script skipped to save space | We have tastefully implemented this into our game and have definitely smoothen the flow of the game. Thanks for the tip! |

### 7.2 Milestone 2
| Suggestion | Response |
| ---------- | -------- |
| Something you might want to take note would be to focus on the group of players that you are creating the game for: eg. If someone doesn't like FPS, don't force them to play FPSes as they will only give bad reviews. | As our game is targeted for people to play with their family and friends (not insisting to replace other different rated games), we have merged mutiple styles into one so that all ages have a share to enjoy it if their interests are a little too different. Take the shooting stage that targets teenagers, math solving questions targetting younger kids and etc. |
| Some things to take note are that if you do want to make players play through the same levels again and again, try and add some more variety, maybe more stages etc.  | We have since added varying difficulties and obstacles so that it does not feel repetitive. At the same time, this is definitely something for future developers to consider when developing our game. |
| It would be great if we could actually see a working demonstration of the game instead. | As provided since the very first milestone, we have always uploaded a playable version of our game on our GitHub repository. We understand that there might be concerns about running a .exe file, so for Milestone 3 and beyond, we have provided a browser-based WebGL deployment of our game using [GitHub Pages](https://marcuslim835.github.io/co-opeDown-WebGL/) |
| Seems more like Button Pressing/Tapping? Or are the chickens going to shoot laser from their mouth to the respective red buttons at the sides? | Having tried out different game styles and interactions between players and the environment in each scene of the game, we have concluded on what best fits a family friendly game while ensuring it's the easiest to play for 2 players while sharing a computer keyboard to play. Having tried to shoot laser from mouth when building the game, we agreed that the shooting movement of the chickens should not be via the mouth as it does not look appealing and logical. Shooting out laser from the hands seem to fit better.|
| Perhaps show the scene of the chicken shooting would be more convincing. ATM, it does not feel special, or harder compared to any of the other games, which makes it worthy to be a 'Boss Fight'. | As this suggestion is for the video, we have taken this suggestion dearly to improve on our video. |
| Unit testing can be done to ensure that all the controls and the game mechanisms are working correctly. | We have implemented user testing leading to Milestone 3 to further improve on the game mechanics. |

## 8 Credits
We would like to thank Lanturnip, ScheduleNeuron, tOThEmOoN, and our advisor Gerald for providing helpful evaluations of our project. <br>
We would also like to extend our appreciation to our friends, families, and the people who have taken their time to participate in our user testings to provide invaluable feedback.

Movement script is based on the one provided by Unity as part of their Standard Assets. <br>
Bat AI script is shared by the [A* Pathfinding Project](https://arongranberg.com/astar/download).
